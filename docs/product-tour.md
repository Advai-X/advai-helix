...
![Helix Work brings tasks, projects, code, tools, and plugins together](images/helix-promo-workspace.png)
...
## 2. Connect the provider that fits your environment

![Helix Work provider configuration for cloud, local, and custom providers](images/helix-promo-providers.png)
...
## 3. Choose and manage models in one place

![Helix Work model management across providers](images/helix-promo-models.png)

You can load the models available to a configured account, add a model by its API ID, enable only the models you want to use, and choose between model families such as GPT, Claude, Gemini, DeepSeek, Qwen, Kimi, GLM, and Grok. This makes it possible to use different models for research, coding, multimodal work, long-context analysis, or local workflows.

Names and specifications shown in the demonstration image are illustrative. The actual models available in Helix come from your configured providers and account access.

## 4. Extend Helix with Plugins, Skills, and MCP

![Helix Work plugin catalog for extending workflows](images/helix-promo-plugins.png)
...
## 5. Understand local usage

![Helix Work local usage dashboard](images/helix-promo-usage.png)

Helix can summarize local task activity, messages, sessions, and model usage so you can understand how the workspace is being used. The values shown in the promotional image are demonstration data.

## 6. Keep local workspace data under control

![Helix Work local storage and backup controls](images/helix-promo-storage.png)

Review the space used by conversations, task workspaces, Skills, browser cache, settings, and backups. Local controls let you export a backup and manage generated data on your Mac.

## 7. Work in one desktop environment
...
## 8. Review outcomes, not just chat messages

![Helix Work creates, verifies, and previews finished deliverables](images/helix-promo-deliverables.png)
...
The example above is a fictional release-readiness demonstration. It shows how a task can end in a verified HTML report while keeping the execution context and delivered file visible in the same workspace.
...
你可以从已配置的 Provider 读取账号可用模型，通过 API Model ID 添加模型，并按任务启用合适的模型。宣传图中的模型名称和参数属于演示配置，实际可用模型取决于供应商和账号权限。

### 4. 通过 Plugins、Skills 与 MCP 扩展

Helix 可以按需增加代码与仓库、浏览器研究、数据分析、可观测性、文档、演示文稿、媒体制作、生产力工具和业务系统等能力。实际访问范围取决于安装配置、凭证、任务范围和你授予的权限。

### 5. 查看本地用量

Helix 可以在本地汇总任务、消息、会话和模型用量，帮助你了解工作区的使用情况。宣传图中的数值为演示数据。

### 6. 管理本地数据

你可以查看对话、任务工作区、Skills、浏览器缓存、设置和备份占用的空间，并在 Mac 上导出备份或管理生成的数据。
...
Helix 不只显示聊天消息，还可以保留执行阶段、工具调用、耗时、交付文件和产物预览。上方发布就绪报告使用的是虚构演示数据，不代表任何真实客户、项目或企业。
